...
char ssid[] = "rocket";        // your network SSID (name)
char pass[] = "rocketlaunch";    // your network password (use for WPA, or use as key for WEP)
    ...
    while (client.connected()) {            // loop while the client's connected
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0 && calltype != CALLTYPE_GET_STATUS) {
            ...
            client.print("button {color: white; width: 200px; height: 200px;");
            client.print("border-radius: 50%; margin: 20px; border: none; font-size: 30px; outline: none; transition: all 0.2s;}");
            ...
            client.print(".cancel{background-color: grey;}");
            client.print("button:hover{cursor: pointer; opacity: 0.7;}");
            client.print("</style>");
            client.print("<div class='container'>");
            client.print("<button class='red' type='submit' onmousedown='location.href=\"/LAUNCH\"'>LAUNCH</button>");
            client.print("</div>");
